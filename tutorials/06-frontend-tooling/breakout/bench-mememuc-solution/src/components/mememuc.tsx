...
interface OmmMemeMUCProps {
  loaded: () => void;
}

const OmmMemeMUC: React.FC<OmmMemeMUCProps> = (props) => {
  ...
  React.useEffect(() => { 
    if (loaded > 0 && loaded === memeState.length) {
      props.loaded()
    }
  }, [loaded, memeState, props])
  ...
  return (<div className="mememuc">
    <ul className="meme-list" onLoad={() => setLoaded(loaded+1)}>{
      memeState.map((meme) => {
